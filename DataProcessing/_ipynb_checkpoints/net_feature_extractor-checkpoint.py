...
from DataProcessing.label_extraction import get_murmur, get_outcome, get_murmurmost, get_murmurlocation, get_s_pitch, \
    get_d_pitch
from HumBugDB.LogMelSpecs.compute_LogMelSpecs import waveform_to_examples
from HumBugDB.LogMelSpecs.deal import padding
...
def net_feature_loader(
    ...
    if recalc_features:
        ...
        torch.save(
            s_pitchs_train, os.path.join(spectrogram_directory, "s_pitchs_train")
        )
        torch.save(
            d_pitchs_train, os.path.join(spectrogram_directory, "d_pitchs_train")
        )
        # 保存一个序列化（serialized）的目标到磁盘
        spectrograms_test, ids_test, s_pitchs_test, d_pitchs_test, murmurs_test, outcomes_test = calc_patient_features(
            test_data_directory
        )
        tmp_l = []
        for i in range(len(s_pitchs_test)):
            p = list(s_pitchs_test[i])
            tmp_l.extend(p)

        tmp_l = np.array(tmp_l)
        #print(tmp_l)
        s_pitchs_test = torch.Tensor(tmp_l)
        # print(s_pitchs_test.shape)
        pos=0
        repeats = torch.zeros((len(s_pitchs_test),))
        for i in range(len(spectrograms_test)):
            for j in range(len(spectrograms_test[i])):
                repeats[pos] = len(spectrograms_test[i][j])
                pos = pos + 1
        # print(repeats)
        # print("---------")
        s_pitchs_test = torch.repeat_interleave(
             s_pitchs_test, repeats.to(torch.int32), dim=0
         )
        '''
        s_pitchs_test = padding(s_pitchs_test, id_train)
        s_pitchs_test = torch.tensor(s_pitchs_test, requires_grad=True)[
                         :, None, :, :
                         ].float()
                         '''
        # s_pitchs_test = torch.Tensor(np.array(s_pitchs_test))
        # print(s_pitchs_train)
        # d_pitchs_test = torch.Tensor(np.array(d_pitchs_test))
        tmp_l = []
        for i in range(len(d_pitchs_test)):
            p = list(d_pitchs_test[i])
            tmp_l.extend(p)

        tmp_l = np.array(tmp_l)
        d_pitchs_test = torch.Tensor(tmp_l)
        # print(d_pitchs_test.shape)
        
        d_pitchs_test = torch.repeat_interleave(
             d_pitchs_test, repeats.to(torch.int32), dim=0
        )
        '''
        d_pitchs_test = padding(d_pitchs_test, id_train)
        d_pitchs_test = torch.tensor(d_pitchs_test, requires_grad=True)[
                         :, None, :, :
                         ].float()
                         '''
        ...
        # )
        # s_pitchs_test = torch.repeat_interleave(
        #     torch.Tensor(np.array(s_pitchs_test)), repeats.to(torch.int32), dim=0
        # )
        # d_pitchs_test = torch.repeat_interleave(
        #     torch.Tensor(np.array(d_pitchs_test)), repeats.to(torch.int32), dim=0
        # )
        ...
        torch.save(s_pitchs_test, os.path.join(spectrogram_directory, "s_pitchs_test"))
        torch.save(d_pitchs_test, os.path.join(spectrogram_directory, "d_pitchs_test"))
    else:
        ...
        s_pitchs_train = torch.load(
            os.path.join(spectrogram_directory, "s_pitchs_train")
        )
        d_pitchs_train = torch.load(
            os.path.join(spectrogram_directory, "d_pitchs_train")
        )
        ...
        s_pitchs_test = torch.load(
            os.path.join(spectrogram_directory, "s_pitchs_test ")
        )
        d_pitchs_test = torch.load(
            os.path.join(spectrogram_directory, "d_pitchs_test ")
        )
    return (
        spectrograms_train,
        murmurs_train,
